...
function createASCIILogo() {
    ...
    // Camera - pull back much further and center view
    const camera = new THREE.PerspectiveCamera(45, 2, 0.1, 2000);
    camera.position.set(0, 0, 15); // Centered and pulled back
    camera.lookAt(0, 0, 0); // Look at center
    
    // Renderer
    const renderer = new THREE.WebGLRenderer({ antialias: true });
    
    // ASCII Effect - larger size for better visibility
    const characters = ' .:-+*=%@#';
    const effect = new THREE.AsciiEffect(renderer, characters, { 
        invert: true, 
        resolution: 0.1 // Lower resolution = bigger ASCII characters
    });
    
    effect.setSize(600, 300); // Larger canvas
    ...
    // Lighting - stronger and better positioned
